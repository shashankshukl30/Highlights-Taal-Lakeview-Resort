        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-12">
          <div className="max-w-[36rem]">
            ...
            <h2 className="font-display text-[34px] md:text-[44px] lg:text-[54px] leading-[1.05] tracking-[-0.025em] mt-3">
              Three suites, a family room — <span className="italic font-light">all lake-facing.</span>
            </h2>
            <p className="mt-5 text-ash text-[16px] leading-[1.6] max-w-[44ch]">
              Book a single room, or take the whole resort for the kind of stay
              you&apos;d only book once a year. Each option has the same view, the
              same pool, the same warm Filipino welcome.
            </p>
          </div>
          <Link href="/rooms" className="btn btn-brand self-start md:self-end">
            See all stay options <ArrowUpRight size={14} className="arrow" />
          </Link>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 md:gap-6">
          {featured.map((room) => (
            <Link key={room.slug} href={`/rooms/${room.slug}`} className="card-room group">
